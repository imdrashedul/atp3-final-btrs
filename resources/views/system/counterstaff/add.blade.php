...
@section('title', 'BTRS - Add Counter Staff')
...
@section('body')
    <div class="card">
        ...
        <div class="card-body">
            <form method="post" action="{{ route('counterstaffadd') }}">
                @csrf
                            ...
                            @if(user()->roleid==roleid_by_name('busmanager'))
                                @if(!empty($counters))
                                    ...
                                    @foreach($counters as $counter)
                                        <option value="{{$counter->id}}" {{ old('counter') ==  $counter->id ? 'selected' : ''}}>{{$counter->name . '['.$counter->location.']'}}</option>
                                    @endforeach
                                @else
                ...
                <div class="form-group row">
                    <div class="col-sm-10 offset-sm-2">
                        <button type="submit" class="btn btn-primary">Add Counter Staff</button>
                    </div>
                </div>
            </form>
        </div>
        ...
        jQuery(function ($) {
            $(document).ready(function () {
                ...
                @if(old('operator', '')!='')
                    ...
                    renderCounter($('select#inputCounter'), '{{old('operator')}}', '{{old('counter')}}');
                @endif
            });
