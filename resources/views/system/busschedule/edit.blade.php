...
@section('title', 'BTRS - Update Bus Schedule')
...
@section('body')
    <div class="card">
        ...
        <div class="card-body">
            <form method="post" action="{{ route('busscheduleedit', ['id' => $busschedule->id]) }}">
                @csrf
                            ...
                            <select name="operator" id="inputOperator" class="form-control @error('operator') is-invalid @enderror">
                                @if(!empty($busmanager))
                                    ...
                                    @foreach($busmanager as $operator)
                                        <option value="{{$operator->id}}" {{ old('operator', $busschedule->bus->operatorid) ==  $operator->id ? 'selected' : ''}}>{{$operator->company}}</option>
                                    @endforeach
                                @else
                            ...
                            @if(user()->roleid==roleid_by_name('busmanager'))
                                @if(!empty($buses))
                                    ...
                                    @foreach($buses as $bus)
                                        <option value="{{$bus->id}}" {{ old('bus', $busschedule->busid) ==  $bus->id ? 'selected' : ''}}>{{$bus->name . '['.$bus->registration.']'}}</option>
                                    @endforeach
                                @else
                            ...
                            @if(user()->roleid==roleid_by_name('busmanager'))
                                @if(!empty($counters))
                                    ...
                                    @foreach($counters as $counter)
                                        <option value="{{$counter->id}}" {{ old('boarding', $busschedule->boardingid) ==  $counter->id ? 'selected' : ''}}>{{$counter->name . '['.$counter->location.']'}}</option>
                                    @endforeach
                                @else
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="from" class="form-control @error('from') is-invalid @enderror" value="{{ old('from', explode('-', $busschedule->route)[0] ) }}" id="inputFrom" placeholder="Enter route (from)">
                        @error('from')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="to" class="form-control @error('to') is-invalid @enderror" value="{{ old('to', explode('-', $busschedule->route)[1]) }}" id="inputTo" placeholder="Enter route (to)">
                        @error('to')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="datetime-local" name="departure" class="form-control @error('departure') is-invalid @enderror" value="{{ old('arrival', date_format(date_create($busschedule->departure),"Y-m-d\TH:i")) }}" id="inputDeparture">
                        @error('departure')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="datetime-local" name="arrival" class="form-control @error('arrival') is-invalid @enderror" value="{{ old('arrival', date_format(date_create($busschedule->arrival),"Y-m-d\TH:i")) }}" id="inputArrival">
                        @error('arrival')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="fare" class="form-control @error('fare') is-invalid @enderror" value="{{ old('fare', $busschedule->fare) }}" id="inputFare" placeholder="Enter fare per seat">
                        @error('fare')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
        ...
        <script type="text/javascript">
            jQuery(function ($) {
                $(document).ready(function () {
                    ...
                    @if(old('operator', $busschedule->bus->operatorid)!='')
                    clearElement($('select#inputBus'));
                    renderCounter($('select#inputBus'), '{{old('operator', $busschedule->bus->operatorid)}}', '{{old('bus', $busschedule->busid)}}', '{{ route('ajax_buses_by_operator') }}', 'Select Bus', 'No Bus Found');
                    clearElement($('select#inputCounter'));
                    renderCounter($('select#inputCounter'), '{{old('operator', $busschedule->bus->operatorid)}}', '{{old('boarding', $busschedule->boardingid)}}', '{{ route('ajax_buscounter_by_operator') }}', 'Select Bus Counter', 'No Bus Counter Found');
                    @endif
                });
